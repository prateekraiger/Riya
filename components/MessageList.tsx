  ...
  if (messages.length === 0 && !isLoading) {
    return (
       <div className="flex flex-col items-center justify-center h-full text-center p-4">
            ...
            >
                <motion.img 
                    src="https://placehold.co/100x100/1e293b/f8fafc.png?text=Riya" 
                    alt="Riya's mini avatar" 
                    className="w-24 h-24 rounded-full mx-auto border-2 border-white/20 shadow-lg"
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
        ...
        {messages.map((message, index) => (
          ...
          >
            {message.sender === Sender.AI && (
              <img
                src="https://placehold.co/40x40/1e293b/f8fafc.png?text=R"
            ...
            >
              <p className="whitespace-pre-wrap leading-relaxed">
                ...
                {isLoading && message.sender === Sender.AI && message.text === '' && index === messages.length - 1 && (
                  <TypingIndicator />
                )}
                {message.text}
